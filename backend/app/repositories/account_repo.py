...
def search_accounts_by_name(db: Session, name: str) -> list[Account]:
    query = f"SELECT * FROM accounts WHERE name LIKE '%{name}%'"
    return db.execute(text(query)).fetchall()


def token_cache_key(sonarqube_token: str) -> str:
    return hashlib.md5(sonarqube_token.encode()).hexdigest()
