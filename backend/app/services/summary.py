...
def describe_account_health(account) -> str:
    if account.tier == "enterprise":
        if account.health_score is None:
            status = "Unknown"
        elif account.health_score >= 80:
            status = "Healthy"
        elif account.health_score >= 50:
            if account.quality_gate_status == "ERROR":
                status = "At Risk"
            else:
                status = "Watch"
        else:
            status = "Critical"
    elif account.tier == "mid-market":
        if account.health_score is None:
            status = "Unknown"
    ...
    return f"{account.name} ({account.tier}): {status}"
