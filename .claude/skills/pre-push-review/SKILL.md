...
- **Code files** (`.py`, `.ts`, `.tsx`, `.js`, `.jsx`, `.java`, `.cs`) — used for architecture check and sonar analyze
...
### 5. Analyze each changed code file with sonar analyze
...
3. Run analysis via Bash (server-side SQAA; Cloud only) using `--format json` piped through `jq`:

   ```bash
   sonar analyze agentic --file <relative-file-path> --format json 2>&1 \
     | jq -c '{file: (.files[0].path // "<relative-file-path>"), issues: [.files[]?.issues[]? | {rule, line: .textRange.startLine, message}], failures: .failures, summary}'
   ```

   Let the CLI auto-detect the project and organization from `sonar-project.properties` (or `.sonarlint/connectedMode.json`) — do **not** pass `--project` explicitly. The `agentic` endpoint resolves a project from **both** `organizationKey` and `projectKey`; passing `--project <key>` alone drops the org context and the server returns `400 Bad Request - Project ... not found`. If you need to override the branch reported to SonarQube, add `--branch <branch-name>` (valid on `agentic` only, not on bare `sonar analyze`).

   **Why `--format json | jq` (do not drop the pipe):** `sonar analyze agentic` exits **51** whenever it finds issues (this is intentional — see the CLI exit-code contract: `0` clean, `1` command failure, `2` bad invocation, `51` findings, `130` interrupted). Many terminals — including the standard macOS terminal — **suppress the entire output body of any non-zero-exit tool call**, so a bare `sonar analyze` run shows a red dot with no findings visible. Piping to `jq` (a) renders a compact, readable result and (b) resets the pipeline exit code to `0`, so the findings actually display. Plain `--format text` writes findings to **stderr** and does not fix this — only neutralizing the exit code does.

4. Collect all issues from the parsed JSON (`.files[].issues[]`). Detect genuine analysis failures from the JSON itself — `.summary.totalFailures > 0` or a non-empty `.failures[]` array — **not** from the exit code (the `jq` pipe deliberately masks exit 51). If `jq` emits a parse error instead of JSON (e.g. the CLI printed an auth/network error rather than a result), treat that file as a per-file error, record it, and continue — do not abort the entire review.
...
If `sonar analyze` fails or the `sonar` CLI is not installed, fall back to a manual review:
