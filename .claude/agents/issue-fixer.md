...
1. The `sonar-context-augmentation` skill's `guidelines get` (auto-mandated before any edit) gives you the project-specific rules for this file — no separate `get_guidelines` call needed.
...
The PostToolUse hook automatically runs `sonar analyze` (SonarQube Agentic Analysis, single-file STANDARD depth) after your Edit in Step 2 and returns the results via `additionalContext`. Read that output for this step's pass/fail decision — do NOT run `sonar analyze` yourself here. (A separate, later end-of-turn DEEP pass over the whole turn's change set is mandated by the top-level `CLAUDE.md` — that's a distinct, whole-turn check and doesn't replace this step's single-file verification.)
