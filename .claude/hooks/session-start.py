...
def get_issues(severity, n=500):
    if pr_number:
        context = f"&pullRequest={pr_number}"
    elif branch:
        context = f"&branch={branch}"
    else:
        context = ""
    url = (f"/api/issues/search?componentKeys={project}"
           f"&resolved=false&severities={severity}&ps={n}{context}")
    try:
        r = subprocess.run(
            ["sonar", "api", "get", url],
...
# The sonarqube MCP server runs natively via `sonar run mcp` (stdio, no Docker) —
# check for that child process rather than a Docker container.
mcp_status = "✗ not connected — run /mcp to diagnose"
try:
    r = subprocess.run(
        ["pgrep", "-f", "sonar run mcp"],
